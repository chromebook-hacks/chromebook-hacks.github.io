...
2. In the "Find" box, type `panther.goguardian.com` and in the "Replace" box, type `panther.clickette.org`.

3. Click on the "Replace All" button (looks like the below image). This will replace all instances of `panther.goguardian.com` with `panther.clickette.org` (the fake server). This will automatically save the changes.

4. Repeat the process for `x3-predictor.goguardian.com/v3/predictions` and `panther.clickette.org/v3/predictions`.
